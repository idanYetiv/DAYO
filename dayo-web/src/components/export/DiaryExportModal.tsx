...
import { X, Download, Share2, Image, BookOpen, Heart, Sparkles } from 'lucide-react'
import { useExportImage, type DiaryExportData, type ExportFormat, type DiaryExportTemplate, type ExportStyle, EXPORT_DIMENSIONS } from '../../hooks/useExportImage'
import { generateDiaryFilename } from '../../lib/exportUtils'
import { toast } from 'sonner'
import DiaryPage from './templates/DiaryPage'
import GratitudeCard from './templates/GratitudeCard'
import HighlightReel from './templates/HighlightReel'
...
const templates: { id: DiaryExportTemplate; name: string; icon: typeof BookOpen }[] = [
  { id: 'diaryPage', name: 'Journal', icon: BookOpen },
  { id: 'gratitudeCard', name: 'Gratitude', icon: Heart },
  { id: 'highlightReel', name: 'Highlights', icon: Sparkles },
]
...
export default function DiaryExportModal({ isOpen, onClose, data }: DiaryExportModalProps) {
  const [template, setTemplate] = useState<DiaryExportTemplate>('diaryPage')
  ...
  const handleExport = async (action: 'download' | 'share') => {
    ...
    const filename = generateDiaryFilename(data.date, format)

    if (action === 'share') {
      const shared = await shareImage(blob, 'My DAYO Diary')
      if (shared) {
  ...
  const renderTemplate = () => {
    ...
    switch (template) {
      case 'diaryPage':
        return <DiaryPage {...props} />
      case 'gratitudeCard':
        return <GratitudeCard {...props} />
      case 'highlightReel':
        return <HighlightReel {...props} />
    }
  }
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-dayo-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-dayo-gray-900">Export Diary Entry</h2>
          </div>
          <button
            onClick={onClose}
